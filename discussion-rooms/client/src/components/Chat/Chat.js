...
const Chat = ({ location }) => {
  ...
  useEffect(() => {
    ...
    const checkRoomExists = async () => {
      let result = await axios.get(
        `${process.env.REACT_APP_API_ENDPOINT}/checkRoomExists/${room}`
      );
  ...
  useEffect(() => {
    ...
    socket.on("remove-from-voice", (user) => {
      setUsersInVoice((usersInVoice) =>
        usersInVoice.filter((x) => x.id !== user.id)
      );
    });
      ...
      getAudio()
        .then((mystream) => {
          ...
          peer.on("open", () => {
            ...
            // won't call myself
            const otherUsersInVoice = usersInVoice.filter(
              (x) => x.id !== socket.id
            );

            peers = otherUsersInVoice.map((u) => {
              // usersInVoice affects this
  ...
  return (
    <div className="outerContainer bgprime">
      ...
      <div className="inMobile bgsec">...scoll down for more</div>
